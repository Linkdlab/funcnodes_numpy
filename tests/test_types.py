...
def test_typestrings():
    ...
    for node in nodes:
        ...
        for ipname, ip in ins.inputs.items():
            t = json.loads(json.dumps(ip.serialize_class()["type"], cls=fn.JSONEncoder))
            try:
                _test_typestring(t)
    ...
    for i, (src, exp, exps) in enumerate(
        [
            ...
            (
                ...
                "Union[_SupportsArray, _NestedSequence, str, _NestedSequence]",
            ),  # 22
        ...
        assert type_to_string(src) == type_to_string(exp), (i, src, exp)
        assert type_to_string(src) == exps, (i, src, exps)


def test_enum_options():
    node = fnp.astype()

    assert node["dtype"].value_options["options"]["keys"] == [
        i.name for i in fnp.DTYPE_ENUM
    ]
    assert node["dtype"].value_options["options"]["values"] == [
        i.value for i in fnp.DTYPE_ENUM
    ]


def test_literal_options():
    node = fnp.newbyteorder()
    assert node["new_order"].value_options["options"]["values"] == [
        "S",
        "<",
        ">",
        "=",
